...
Stories.getInitialProps = async (ctx) => {
  try {
    ...
    if (ctx.query.theloai === 'trending') {
      detail = {
        title : 'Danh sách truyện full mới nổi hấp dẫn | Webtruyen',
    ...
    if (ctx.query.theloai === 'moi-nhat') {
      detail = {
        title :  'Truyện full đang ra mới nhất, cập nhật liên tục | Webtruyen',
    ...
    if (ctx.query.theloai === 'truyen-full') {
      detail = {
        title :  'Danh sách truyện full đã hoàn cuốn hút | Webtruyen',
    ...
    if (ctx.query.theloai === 'xem-nhieu-nhat') {
      detail = {
        title :  'Tuyển tập bộ truyện full được đọc nhiều | Webtruyen',
    ...
    if (ctx.query.theloai === 'hot') {
      detail = {
        title : 'Danh sách những bộ truyện HOT FULL cập nhật mới nhất | Webtruyen',
