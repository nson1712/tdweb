...
const StoryItem = ({item, direction = 'col', chapterSlug, currentChap, isBookMarked, fromSlide, fromSearch, saveStory}) => {
  ...
  return (
    <div className={classNames('flex flex-col cursor-pointer', direction === 'row' && 'flex-row items-start')}
      onClick={() => {
        if(saveStory) {
          saveStory(item.slug)
        }
    ...
    >
      ...
      <div className={classNames(direction === 'row' && 'flex-1')}>
        <div className='flex items-center justify-between mb-[4px]'>
          <a className={classNames('text-[14px] font-semibold leading-[16px] main-text mb-0 line-clamp-2', direction === 'col' && 'mb-[4px]  h-[32px]', direction === 'row' && 'text-[12px] mb-[4px]')} 
            href={`${chapterSlug ? `/${item.slug}/${chapterSlug}` : `/${item.slug}`}`} 
            title={`Truyện ${item.title}`}>
            {item.title}
          </a>
          { isBookMarked
            && <img src='/images/book-mark-active.svg' className='w-[24px]' alt="Đã thêm vào thư viện" title="Thêm vào thư viện"/>
          }
          
        </div>
        
        <div className={classNames(direction === 'row' && 'flex items-center', fromSlide === true && 'hidden md:flex', fromSearch && 'flex-col flex-start')}>
          <p className={classNames('text-[12px] font-medium leading-[16px] label-text', direction === 'col' && 'mb-[4px]', direction === 'row' && 'mb-[8px] mr-[32px]', fromSearch &&  'mb-[4px]')}>
            {formatStringToNumber(item.totalView)} lượt xem
          </p>
          <p className={classNames('text-[12px] font-medium leading-[16px] label-text flex items-center', direction === 'col' && 'mb-0', direction === 'row' && 'mb-[8px]', fromSearch &&  'mb-[4px]')}>
            {formatStringToNumber(item.rate)}/5
            <img src='/images/star.svg' className='w-[12px] ml-[4px]' alt='Đánh giá truyện' title='Đánh giá truyện'/>
          </p>
        </div>
          ...
          && <>
            ...
            <h3 className='secondary-text text-[12px] font-medium leading-[16px] mt-[4px] mb-0'>
              ...
              {item?.categories?.slice(0, 3).map((item, i) => (
                  <Link href={`/the-loai/${item.code}`} title={`Thể loại truyện ${item.name}`}
                    onClick={(e) => {
                      e.preventDefault()
                      e.stopPropagation()
                    }}
                  >
                    <a className='secondary-text text-[12px] font-medium leading-[16px] mt-[4px] mb-0 mr-[6px]' title={`Thể loại truyện ${item.name}`}
                       onClick={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
        ...
        {currentChap
          && <>
            <div className='mb-[6px] h-[4px] rounded-[2px] progress-color relative mt-[8px] max-w-[245px]'>
              <div className='absolute left-0 top-0 bottom-0 progress-active-color'
                style={{
