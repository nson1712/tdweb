...
import { formatStringToNumber, formatDateTime } from '../../utils/utils'
    ...
    try {
      if(cash === 0) {
        ...
        if ('00' !== result?.data.code) {
          ...
          Router.push(`/nap-kim-cuong/thong-tin-chuyen-khoan?accountName=${result?.data.accountName}&accountNumber=${result?.data.accountNumber}&amount=${result?.data.amount}&description=${result?.data.contentTransfer}&qrCode=${result?.data.qrCode}&expriedAt=${timePlusTenMinutes.toISOString()}&order=${result?.data.orderCode}&paymentId=${result?.data.paymentLinkId}`)
  ...
  return (
    <CommonLayout >
        ...
        <div className='relative max-w-[768px] mx-auto bg-white mt-[16px] md:pt-[88px] flex flex-col justify-center text-second-color'>
          ...
          <div className='pl-[20px] pr-[20px] mb-[20px]'>
            <div className="max-w-[450px] w-full mx-auto alert alert--secondary admonition_LlT9 pl-[20px] pr-[20px]">
              ...
              <div class="admonitionContent_S0QG"><p>Mã khách hàng: là mã TD.... được lấy từ màn hình "Tài Khoản" trên App Toidoc</p></div>
              <a onClick={(e) => setShowModal(true)} style={{'color': '#0693ee', 'textDecoration': 'underline'}}>
                Xem hướng dẫn
              </a>
            </div>
            <Form onSubmit={submitForm(handleRequestPayment)} className='max-w-[450px] w-full mx-auto'>
              ...
              />
              <p className='text-[14px] font-semibold'>2. Bấm chọn gói nạp bên dưới</p>
              {showWarningPackage &&
              <p className='text-[14px] text-red'>* Bạn phải lựa chọn gói kim cương</p>}
              <div className='btnContainer'>
                {packages.slice(0, 3).map((item, index) => (
                  ...
                  >
                  <div>
                    {`CK ${item.deposit} -> Nhận ${item.label}`} <RedDiamondIcon className='float-right ml-[5px]'/>
                  </div>
                  </Button>
                ))}

                {showMorePackage && packages.slice(3, 6).map((item, index) => (
                  <Button
                    key={index+3}
                    className={`btn btnSelectDiamond w-[350px] h-[50px] pt-[10px] ${
              ...
              <p className='text-[16px] font-bold'>Số tiền bạn cần chuyển là: <span className='text-[20px] font-bold main-text'>{formatStringToNumber(cash)} VNĐ</span></p>

              <Button className='btnMain btnSecond'
                type='submit'
                loading={loading}
              >
      ...
      {showModal && <ModalComponent
        ...
        isCountDown={false}
      >
        <img src='https://media.truyenso1.xyz/ads/guide-customer-code-1.jpg' className='imgBanner'/>
      </ModalComponent>}
    </CommonLayout>
  )
}
...
const validate = (values) => {
  ...
  } else if (!values.customerCode.startsWith('TD') || values.customerCode.length < 12) {
